...
import BussinessProfile from '../../../../admin/src/admin/pages/bussiness-profile/index';
...
export class UserController {
  ...
  static async updateProfile(req, res, next) {
    ...
    try {
      ...
      user.licenseImage =typeof  req?.body?.licenseImage === "string"?  req?.body?.licenseImage : req?.files?.licenseImage[0].originalname;
      user.projectImages =req?.body?.projectImages || req?.files?.projectImages?.map((item) => item.originalname)
      console.log("projectImages",user.projectImages,req?.files?.projectImages)
      await user.save();
      return _RS.ok(
        res,
        "SUCCESS",
  ...
  static async updateBussinessProfile(req, res, next) {
    ...
    try {
      ...
      if(BussinessProof){
        ...
        BussinessProof.COGS = typeof req?.body?.COGS == "string"? req?.body?.COGS: req?.files?.COGS[0].originalname;
        BussinessProof.AOI =typeof  req?.body?.AOI === "string"?  req?.body?.AOI : req?.files?.AOI[0].originalname;
        BussinessProof.OA =typeof  req?.body?.OA === "string"?  req?.body?.OA : req?.files?.OA[0].originalname;
        BussinessProof.stateIDFront =typeof  req?.body?.stateIDFront === "string"?  req?.body?.stateIDFront : req?.files?.stateIDFront[0].originalname;
        BussinessProof.stateIDBack =typeof  req?.body?.stateIDBack === "string"?  req?.body?.stateIDBack : req?.files?.stateIDBack[0].originalname;
        BussinessProof.COI =typeof  req?.body?.COI === "string"?  req?.body?.COI : req?.files?.COI[0].originalname;
        ...
        saveTheBussiness.COGS = typeof req?.body?.COGS == "string"? req?.body?.COGS: req?.files?.COGS[0].originalname;
        saveTheBussiness.AOI =typeof  req?.body?.AOI === "string"?  req?.body?.AOI : req?.files?.AOI[0].originalname;
        saveTheBussiness.OA =typeof  req?.body?.OA === "string"?  req?.body?.OA : req?.files?.OA[0].originalname;
        saveTheBussiness.stateIDFront =typeof  req?.body?.stateIDFront === "string"?  req?.body?.stateIDFront : req?.files?.stateIDFront[0].originalname;
        saveTheBussiness.stateIDBack =typeof  req?.body?.stateIDBack === "string"?  req?.body?.stateIDBack : req?.files?.stateIDBack[0].originalname;
        saveTheBussiness.COI =typeof  req?.body?.COI === "string"?  req?.body?.COI : req?.files?.COI[0].originalname;
